...
\item Incompressibility: prevents a functionally equivalent deduction \textcolor{red}{with a significantly smaller memory footprint}.
...
\frame
{
	...
	\begin{enumerate}[1.]
		...
		\item nearly 3,240 encryptions can help to collect 25 elements in $\mathcal{W}$.
	\end{enumerate}
	Such that,
	\begin{enumerate}[1.]
		\item at least about 3,265 encryptions (3,240 times for obtaining $\mathcal{W}$
		...
		\item totally at least about 6,530 encryptions can extract the 16-byte first-round key.
	\end{enumerate}
}
...
\frame
{
	...
	In 2019, Lee \textit{et al.} [eprint 2019/959] proposed a table redundancy scheme based on Chow \textit{et al.}'s white-box AES for protecting against DFA.
	\\[2ex]
...
\frame
{
	Since $x$ takes 256 different values and the encoding is a fixed bijection, the 256 pairs of each ($y_{l},y_{r}$) can be collected.
	\\[2ex]
...
\frame
{
	...
	\begin{itemize}
		...
		\item Getting a fault-free ciphertext with a chosen plaintext $P$.
...
\frame
{
	...
	\begin{enumerate}[1.]
		...
		\item repeat the steps for the remain columns, such that totally at least about 20 times encryption can extract the 16-byte tenth-round key.
	\end{enumerate}
}
